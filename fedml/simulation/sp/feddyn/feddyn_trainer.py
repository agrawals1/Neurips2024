...
class FedDynTrainer(object):
    def __init__(self, dataset, model, device, args):
        ...
        self.server_state = {
            ...
            for key, params in self.model_trainer.get_model_params().items()
        }
    ...
    def train(self):
        ...
        for round_idx in range(self.args.comm_round):
            ...
            # m = self.args.client_num_in_total
            # self.server_state = {
            #     key: prev_h - self.args.feddyn_alpha * 1 / m * sum(
            #         local_model_params[key] - old_params for (local_sample_number, local_model_params) in model_list)
            #     for (key, prev_h), old_params in zip(self.server_state.items(), self.trainer.get_model_params().values())
            # }
